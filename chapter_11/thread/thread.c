...
void thread_create(struct task_struct* pthread, thread_func funcion, void* func_arg) {
    ...
    kthread_stack->eip = kernel_thread; // 通过switch_to调用kernel_thread
...
struct task_struct* thread_start(char* name, int prio, thread_func function, void* func_arg) {
    struct task_struct* thread= get_kernel_pages(1);    //  申请线程PCB存储空间
...
void schedule(void) {
    ...
    if (cur->status == TASK_RUNNING) {
        ...
        cur->ticks = cur->priority; // 重置线程时间片
...
void thread_unblock(struct task_struct* pthread) {
    ...
    if (pthread->status != TASK_READY) {
        ...
        list_push(&thread_ready_list, &pthread->general_tag);   // 放到队列头部 实现快速调度
        pthread->status = TASK_READY;
    }
    intr_set_status(old_status);
}
